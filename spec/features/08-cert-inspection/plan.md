...
The concrete driving example is a self-signed **SLH-DSA (SPHINCS+) post-quantum root CA**, generated
in-repo with openssl (see *Fixture policy* below). `openssl` shows it as: `Signature Algorithm:
SLH-DSA-SHA2-128s`; `KeyUsage = Certificate Sign, CRL Sign` (critical, per the generation recipe);
`BasicConstraints` critical `CA:TRUE`; `SAN DNS:slh-dsa-test-root`; `subject = issuer = CN=SLH-DSA Test
Root, C=SE, O=mini-x509-linter testdata`. The linter parses it fine structurally (reports OK) but
cannot display any of these values. The signature/public-key algorithm may or may not be known to
`oid-registry` (the SLH-DSA-SHA2-128s OID is `2.16.840.1.101.3.4.3.20`); whichever it is, the summary
must degrade gracefully and show the raw dotted OID rather than failing — the human-readable name is
best-effort.

### Fixture policy (hard rule)

**All test fixtures for mini-X509-Linter MUST be generated with `openssl`, NEVER vendored from or
generated by the user's external `cert-bar` tool.** Rationale: this linter will be used to validate
`cert-bar`'s output, so it must remain an INDEPENDENT oracle — sourcing fixtures from `cert-bar` would
create a circular validation dependency. `cert-bar` may be mentioned only as out-of-scope; it is never
a fixture source. The verified environment has openssl 3.6.2 at `/opt/homebrew/bin/openssl`, which
supports SLH-DSA natively, so the PQC fixture is generated locally and reproducibly via `openssl`.
...
- **PQC-friendliness (explicit requirement).** The signature and public-key algorithms may be unknown
  to `oid-registry` (e.g. SLH-DSA). `signature_algorithm()` MUST return the raw dotted OID string
  whenever the name is unknown; the human-readable name is best-effort. The summary MUST degrade
  gracefully — always display the raw dotted OID string plus a sensible label — and MUST NOT error,
  panic, or omit the field. Whether `oid-registry` 0.8 (with `x509-parser` 0.18) happens to recognize
  the SLH-DSA OID is an implementation detail the developer/tester will discover; the test MUST assert
  on **OID presence** (always works) and treat the human name as best-effort. This is exercised by a
  dedicated test against the openssl-generated SLH-DSA fixture.
...
- `slh_dsa_root_ca.pem` — NEW committed PQC fixture, a self-signed SLH-DSA root CA **generated with
  openssl** (see *Open Decisions* for the recipe). NOT sourced from cert-bar.
- `testdata/generate.sh` — extended with the SLH-DSA recipe so the fixture is reproducible.
...
3. **PQC fixture provenance — openssl only.** The tester (task 03) generates an in-repo
   `testdata/slh_dsa_root_ca.pem` with **openssl 3.6.2**, which supports SLH-DSA natively (verified:
   `openssl genpkey -algorithm SLH-DSA-SHA2-128s` then `openssl req -x509 -new -key … -addext
   basicConstraints=critical,CA:TRUE -addext keyUsage=critical,keyCertSign,cRLSign -addext
   subjectAltName=DNS:slh-dsa-test-root`). The recipe goes into `testdata/generate.sh` for
   reproducibility, and provenance is documented in the test header. It is **never** sourced from
   `cert-bar` (the linter must remain an independent oracle for cert-bar's output). The fixture only
   needs to PARSE structurally — its algorithm OID possibly being unknown to `oid-registry` is the whole
   point (exercises `signature_algorithm()`'s raw-OID fallback).
