...
3. CLI surface: `<PATH>...`, `--format text|json`, `--source rfc5280,cabf_br,hygiene`,
   `--min-severity`, `--fail-on`, `--chain`. (Document `--from-host`/`--sni`/`--timeout`
   only as "added in the fetch feature" or leave a placeholder; that lands in feature 07.)
...
5. Example invocations with sample output (text + JSON).
